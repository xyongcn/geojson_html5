...
var fs = require("fs");
//var sm_geojson = require('osm-and-geojson');
...
//console.log("nodejs server.js 16");
var tileName = process.argv.slice(2);
osmTileName = tileName.toString();
jsonTileName = tileName.toString()+'.json';

//console.log(osmTileName.toString());
var osm_data = fs.readFileSync(osmTileName);
//console.log(osm_data);
var data = osm_data.toString();
//console.log(data);
...
//console.log("程序执行完毕。" + JSON.stringify(geojson));

fs.writeFile(jsonTileName, JSON.stringify(geojson),  function(err) {
   if (err) {
...
/*
   console.log("读取写入的数据！");
   fs.readFile('16.json', function (err, data) {
      if (err) {
         return console.error(err);
      }
